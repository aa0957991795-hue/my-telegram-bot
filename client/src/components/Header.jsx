...
export default function Header() {
  ...
  const handleNativeSelectChange = (e) => {
    ...
    if (found) {
      updateCity(found.id, found.name);
    }
  };

  return (
    <header className="sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 px-4 py-2.5 z-40 flex items-center justify-between shadow-sm">
      {/* City Dropdown / Selector Pill */}
      <div className="relative flex items-center">
        {/* Visible styled pill with native select overlay for 100% mobile compatibility */}
        <div className="relative flex items-center gap-1.5 text-xs font-semibold text-slate-800 dark:text-slate-100 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-3 py-1.5 rounded-full transition-all border border-slate-200/60 dark:border-slate-700/60 shadow-sm cursor-pointer">
          <span className="text-emerald-600 dark:text-emerald-400">📍</span>
          <span>{currentCityName}</span>
          <span className="text-[10px] text-slate-400">▼</span>

          {/* Native HTML Select invisible overlay for flawless mobile picker */}
          <select
            value={user?.cityId || 1}
      ...
      <div className="flex items-center gap-2">
        {user && (
          <button
            ...
            className="flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 border border-emerald-200/60 dark:border-emerald-800/60 px-3 py-1.5 rounded-full text-xs font-bold text-emerald-700 dark:text-emerald-400 transition-all active:scale-95"
          >
            <span>💰</span>
